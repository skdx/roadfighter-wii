...
int CRoadFighter::interlevel_cycle(void)
{
	if (state_timmer==0) {
        ...
        // "Cut" factor for scoreboard width in 2 player screen
        float SCOREBOARD_CUT_FACTOR = 0.88f;
		if (n_players==1) desired_scoreboard_x=SCREEN_X-(scoreboard_sfc->w-1);
                    else desired_scoreboard_x=SCREEN_X-(SCOREBOARD_CUT_FACTOR*scoreboard_sfc->w);
