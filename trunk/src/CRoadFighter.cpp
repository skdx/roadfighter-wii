...
#ifdef _WII
#include "wii_utils.h"
#define IMG_Load(x) RootDirPrefix_IMG_Load(x)
...
CRoadFighter::CRoadFighter(void)
{
    ...
    char COMICBD_FONT_PATH[256];
    insert_rootdir_prefix("fonts/tanglewo.ttf", TANGLEWO_FONT_PATH);
    insert_rootdir_prefix("fonts/comicbd.ttf", COMICBD_FONT_PATH);

	font1=TTF_OpenFont(COMICBD_FONT_PATH, 16);
	font2big=TTF_OpenFont(TANGLEWO_FONT_PATH, FONT_SIZE);
	font2medium=TTF_OpenFont(TANGLEWO_FONT_PATH, int(FONT_SIZE*0.8));
	font2small=TTF_OpenFont(TANGLEWO_FONT_PATH, int(FONT_SIZE*0.65));
...

	{
		SDL_Color c;
		char *text="        "
				   "Game originally created for the RETROREMAKES REMAKE COMPETITION 2003"
				   "        "
				   "PROGRAMMING: Santi Ontan (Brain)    "
				   "GRAPHICS: Miikka Poikela (McBain)    "
				   "MUSIC/SFX: Jorrith Schaap (Jorito)    "                   
				   "BETA TESTING: Jason Eames (JEames), Miikka Poikela (McBain), Jorrith Schaap (Jorito), Santi Ontan (Brain)    "				   
                   "WII PORT: Alexandre Moreira    "
                   "        ";
		c.r=c.g=c.b=128;
...
void CRoadFighter::scoreboard_draw(int x,int y,SDL_Surface *screen)
{
	...
	if (game!=0) {
		...
		while(l.Iterate(speed)) {
			tmp=int(146*(float(-*(speed))/MAX_SPEED));
			tmp2=(41-l.Length())/l.Length();

			for(i=458;i>458-tmp;i-=2) {
				r.x=(x+35)+(tmp2+1)*j;
				r.y=i;
				r.w=tmp2;
				r.h=1;
	...
	if (game!=0) {
		...
		while(l.Iterate(fuel)) {
            tmp=int(146*(float(*fuel)/MAX_FUEL));
            tmp2=(41-l.Length())/l.Length();
	...
	/* Draw the mini-map: */ 
	{
        ...
        r.x=scoreboard_x+52;
        r.y=121;
		r.w=scoreboard2_sfc->w;
		r.h=scoreboard2_sfc->h;
	...
	if (game!=0 && (state_timmer>>3)&0x01!=0) {
		...
		while(l.Iterate(pos)) {
            ...
            if (j==0) minicar1_tile->draw(car_x+17,car_y+43,screen);
            if (j==1) minicar2_tile->draw(car_x+18,car_y+43,screen);
	...
	if (n_players==1) {
        ...
        r.x=(-scoreboardleft_sfc->w) + scoreboardleft_sfc->w*f;
		r.y=0;
